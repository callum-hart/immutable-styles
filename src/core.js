...
function createCSS(styles, includeVersion = false) {
  Array.isArray(styles)
      ? styles.forEach(block => parseStyles(block))
      : parseStyles(styles);

  parseAst();
  return makeCSS(includeVersion);
}
...
function makeCSS(includeVersion) {
  let CSS = includeVersion ? `/* Generated by Immutable Styles ${VERSION} */\n` : BLANK;
  ...
  for (var ref of AST.keys()) {
    ...
    AST.get(ref).filter(({styles}) => styles !== BLANK)
      ...
      if (
        ...
        CSS += `${selector}${OPEN_BRACE}${styles}`;
      } else {
        ...
        CSS += `@media${ranges.join(`${SPACE}and${SPACE}`)}${OPEN_BRACE}`;
